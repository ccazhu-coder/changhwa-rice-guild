...
function renderPoliciesPage(){
  ...
  function renderMembers(){var box=qs('memberGrid');if(!box)return;var srchEl=document.getElementById('memberSearch');var lvlEl=document.getElementById('memberLevel');var regEl=document.getElementById('memberRegion');var cntEl=document.getElementById('memberCount');staticOrFetch('members').then(function(d){var all=d.members||[];if(!all.length){box.innerHTML='<div class="member-empty">暫無會員資料</div>';return;}var areas=[];all.forEach(function(m){if(m.area&&areas.indexOf(m.area)===-1)areas.push(m.area);});areas.sort();if(regEl&&!regEl.dataset.filled){areas.forEach(function(a){var op=document.createElement('option');op.value=a;op.textContent=a;regEl.appendChild(op);});regEl.dataset.filled='1';}var CHANGHUA_AREAS=['彰化市','鹿港鎮','和美鎮','線西鄉','伸港鄉','福興鄉','秀水鄉','花壇鄉','芬園鄉','員林市','溪湖鎮','大村鄉','埔鹽鄉','埔心鄉','永靖鄉','社頭鄉','二水鄉','田中鎮','北斗鎮','田尾鄉','埤頭鄉','芳苑鄉','大城鄉','竹塘鄉','溪州鄉','二林鎮'];function applyFilter(){var q=(srchEl?srchEl.value.trim():'').toLowerCase();var lv=lvlEl?lvlEl.value:'';var ar=regEl?regEl.value:'';var res=all.filter(function(m){if(lv&&(m.grade||'')+'級'!==lv)return false;if(ar&&(m.area||'')!==ar)return false;if(q&&!(m.name||'').toLowerCase().includes(q)&&!(m.owner||'').toLowerCase().includes(q)&&!(m.area||'').toLowerCase().includes(q)&&!(m.address||'').toLowerCase().includes(q)&&!(m.id||'').toLowerCase().includes(q))return false;return true;});var hasFilter=!!(q||lv||ar);var display=hasFilter?res:res.slice(0,48);if(cntEl)cntEl.textContent=hasFilter?('共 '+res.length+' 筆'):('顯示 '+display.length+' / 共 '+res.length+' 筆');box.innerHTML=display.length?display.map(function(m){var areaDisp=CHANGHUA_AREAS.indexOf(m.area||'')>-1?('彰化縣・'+m.area):(m.area||'');return('<div class="member-card" style="background-color:var(--rice-2);border:1px solid var(--gold-2)">'+'<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:12px">'+'<h3 style="font-size:1.15rem;font-weight:800;color:var(--green);line-height:1.4;margin:0">'+esc(m.name||'')+'</h3>'+'<span style="flex-shrink:0;background:var(--green);color:#fff;font-size:.78rem;font-weight:700;padding:5px 16px;border-radius:999px">'+esc(m.grade||'')+'級</span>'+'</div>'+'<div style="display:flex;align-items:center;margin-bottom:14px">'+'<span style="width:7px;height:7px;background:var(--gold);transform:rotate(45deg);flex-shrink:0"></span>'+'<span style="flex:1;height:1px;background:var(--gold-2);margin-left:5px"></span>'+'</div>'+'<div style="display:grid;gap:9px">'+(m.owner?'<div style="display:flex;gap:10px;font-size:.95rem;line-height:1.5"><span style="font-weight:800;color:var(--green);min-width:60px;flex-shrink:0">負責人</span><span style="color:var(--gold);flex-shrink:0">|</span><span style="color:#444;flex:1;min-width:0">'+esc(m.owner)+'</span></div>':'')+'<div style="display:flex;gap:10px;font-size:.95rem;line-height:1.5"><span style="font-weight:800;color:var(--green);min-width:60px;flex-shrink:0">所在地</span><span style="color:var(--gold);flex-shrink:0">|</span><span style="color:#444;flex:1;min-width:0">'+esc(areaDisp)+'</span></div>'+'<div style="display:flex;gap:10px;font-size:.95rem;line-height:1.5"><span style="font-weight:800;color:var(--green);min-width:60px;flex-shrink:0">地址</span><span style="color:var(--gold);flex-shrink:0">|</span><span style="color:#444;flex:1;min-width:0">'+esc(m.address||'')+'</span></div>'+'</div>'+'</div>');}).join(''):'<div class="member-empty">查無符合的會員</div>';}applyFilter();if(srchEl)srchEl.addEventListener('input',applyFilter);if(lvlEl)lvlEl.addEventListener('change',applyFilter);if(regEl)regEl.addEventListener('change',applyFilter);});}
