...
function renderPoliciesPage(){
  ...
  function renderCommittees(){var box=qs('committeesGrid');if(!box)return;staticOrFetch('committees').then(function(d){var list=d.committees||[];box.innerHTML=list.length?'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px">'+list.map(function(c){return('<div style="background:#fff;border:1px solid #e5e5e5;border-radius:14px;padding:20px">'+'<h4 style="font-size:.93rem;color:#1d6b2e;margin:0 0 8px;font-weight:700">'+esc(c.name)+'</h4>'+'<div style="font-size:.78rem;color:#666;margin-bottom:8px">主委：<strong style="color:#1a1a1a">'+esc(c.chair)+'</strong>'+(c.viceChair?'&ensp;副主委：<strong style="color:#1a1a1a">'+esc(c.viceChair)+'</strong>':'')+'</div>'+'<p style="font-size:.8rem;color:#666;line-height:1.7;margin:0">'+esc(c.duties)+'</p>'+'</div>');}).join('')+'</div>':'<p style="color:#aaa">暫無資料</p>';});}
